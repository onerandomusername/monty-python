...
class BatchParser:
    ...
    async def get_markdown(self, doc_item: _cog.DocItem) -> Optional[str]:
        ...
        if doc_item not in self._item_futures and doc_item not in self._queue:
            ...
            async with bot.bot.bot.http_session.get(doc_item.url, raise_for_status=True) as response:
                soup = await bot.bot.bot.loop.run_in_executor(
                    None,
                    BeautifulSoup,
    ...
    async def _parse_queue(self) -> None:
        ...
        try:
            while self._queue:
                ...
                try:
                    markdown = await bot.bot.bot.loop.run_in_executor(None, get_symbol_markdown, soup, item)
